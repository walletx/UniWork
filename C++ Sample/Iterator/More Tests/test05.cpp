...
int main(void) {
  ...
  // Finding Elements  
  find_in_tree(bti, 5);
  find_in_tree(bti, -2447);
  find_in_tree(bti, 198);
  find_in_tree(bti, 655);
  ...
  find_in_tree(bti, 5);
  find_in_tree(bti, -2447);
  find_in_tree(bti, 1002);
  find_in_tree(bti, 199);
  find_in_tree(bti, 655);
  find_in_tree(bti, 1001);
  find_in_tree(bti, 1000);
  find_in_tree(bti, 99);
  find_in_tree(bti, 199);
  find_in_tree(bti, 333);
  find_in_tree(bti, 777);
  ...
  std::cout << btui << std::endl;

  find_in_tree(btui, static_cast<unsigned int>(653));
  ...
  for (auto it = btcc.begin(); it != btcc.end(); ++it) {
      std::cout << *it << std::endl;
  }
  
  std::cout << btcc << std::endl;
  ...
  std::cout << btch << std::endl;
  
  for (auto it = btch.begin(); it != btch.end(); ++it) {
      std::cout << *it << std::endl;
  }

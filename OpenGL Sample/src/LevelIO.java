...
public class LevelIO {
    ...
    public static Terrain load(File mapFile) throws FileNotFoundException, JSONException {
        ...
        if (jsonTerrain.has("others")) {
        	...
        	for (int i = 0; i < jsonOthers.length(); i++) {
                ...
                JSONArray jsonVertices = jsonOther.getJSONArray("vertices");
                float[] vertices = new float[jsonVertices.length()];
    
                for (int j = 0; j < jsonVertices.length(); j++) {
                	vertices[j] = (float) jsonVertices.getDouble(j);
                }
    
                JSONArray jsonTexture = jsonOther.getJSONArray("texture");
                float[] texture = new float[jsonTexture.length()];
                for (int j = 0; j < jsonTexture.length(); j++) {
                	texture[j] = (float) jsonTexture.getDouble(j);
                }       

                terrain.addVBOObject(vertices, texture);
            }
        }
    ...
    public static void save(Terrain terrain, File file) throws IOException, JSONException {
        ...
        for (VBOObject o : terrain.VBOObjects()) {
            JSONObject j = new JSONObject();
        
            JSONArray vertices = new JSONArray();
            int n = o.getVerticesNum();
        
            for (int i = 0; i < n; i++) {
                vertices.put(o.verticesArray()[i]);
            }
            j.put("vertices", vertices);
        
            JSONArray texture = new JSONArray();
            n = o.getTextureNum();
        
            for (int i = 0; i < n; i++) {
            	texture.put(o.textureArray()[i]);
            }
            j.put("texture", texture);
